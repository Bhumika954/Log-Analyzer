...
In general, it's a good idea to look at [LogAnalyzer on OBS](https://build.opensuse.org/package/show/home:pbek:LogAnalyzer/desktop)
for repepositories for the application.

## Install via Nix

You can install LogAnalyzer with the [Nix package manager](https://nixos.wiki/wiki/Nix_package_manager)
on [NixOS](https://nixos.org/) and other platforms, the package name is
[loganalyzer](https://search.nixos.org/packages?channel=unstable&show=loganalyzer).

See [LogAnalyzer on nix](https://search.nixos.org/packages?channel=unstable&show=loganalyzer)
